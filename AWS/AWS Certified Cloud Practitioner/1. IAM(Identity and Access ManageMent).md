...
고객이 직접 생성하고 관리하는 정책.
* 커스터마이징 가능 : AWS Managed Policies 과는 다르게 수정할 수 있음. 
* 버전 관리 : 정책(Policy)의 여러 버전을 만들고 관리 할 수 있음. (커스터마이징의 주체가 고객이니 당연)
* 재사용 가능: 여러사용자, 그룹, 역할에 쉽게 할당 할 수 있다. (재사용 뿐만 아니라 상황에 맞게 커스텀가능)
* 관리 책임 : 정책의 관리와 유지보수는 고객의 책임                    (커스터마이징의 주체가 고객이니 당연)
<u>EX) : 특정 S3 버킷에 대한 읽기/쓰기 권한을 부여하는 정책 등 </u>

#### 3. Inline Policies
특정 사용자, 그룹, 역할에 직접 연결된 정책. 정책이 연결된 엔티티와 생명주기를 공유한다. 
* 직접 연결 : 특정 엔티티에 직접 연결되며, 해당 엔티티 삭제되면 정책도 삭제됨. 
* 고유성 : 재사용되지 않으며, 특정 엔티티에만 적용됨.
* 관리 부담 : 여러 엔티티에 걸쳐 동일한 정책을 적용할 때 관리부담이 증가 할 수 있음.
* 유연성 : 특정 상황에 맞게 
