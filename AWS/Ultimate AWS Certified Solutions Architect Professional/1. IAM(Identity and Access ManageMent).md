...
에를 들면 Lambda function이 동작하면 Kinesis Data Streams 에서 S3 Bucket의 데이터를 수집하는 flow가 있다면 이러한 일련의 API Call은 CloudTrail 에 기록된다. 이 때 생성된 로그는 IAM Access Analyzer에 의해 검토되고 정책을 생성한다. 
