...
- 하지만 Lambda 함수 실패 시 재시도 기능이 없어 이벤트 손실이 발생할 수 있습니다.

**S3 - SNS - SQS - Lambda 아키텍처**

- S3 버킷에서 이벤트가 발생하면 SNS 토픽으로 알림이 전송됩니다.
- SNS 토픽은 SQS 대기열을 트리거하여 이벤트를 전달합니다.
- SQS 대기열은 Lambda 함수를 트리거하여 이벤트를 처리합니다.
- 이 아키텍처는 SQS의 재시도 기능과 Dead Letter Queue(DLQ)를 활용하여 이벤트 손실을 방지할 수 있습니다.
- 하지만 이벤트 처리 지연이 증가할 수 있습니다.

**요약**

- S3 - SNS - Lambda 아키텍처는 간단하고 빠르지만 이벤트 손실 위험이 있습니다.
- S3 - SNS - SQS - Lambda 아키텍처는 이벤트 손실 방지를 위해 SQS와 DLQ를 사용하지만, 이벤트 처리 지연이 증가할 수 있습니다.

따라서 사용 사례와 요구사항에 따라 적절한 아키텍처를 선택해야 합니다.