...
`npm run dev`
 ...
 환경 변수 설정
Node.js 17 이상 버전에서는 OpenSSL 3.0이 기본으로 사용되며, 이는 일부 해시 알고리즘을 기본적으로 지원하지 않는다. 이로 인해 Webpack이나 기타 도구들에서 `crypto` 모듈을 사용할 때 문제가 발생할 수 있다. 이러한 문제를 해결하기 위해 `openssl-legacy-provider` 옵션을 사용한다.
